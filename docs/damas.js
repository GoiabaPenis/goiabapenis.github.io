...
function procurarMovimentos(peca) {
    ...
    if (celulaCima) { 
        ...
        if (celulaCima.cells[localPeca-1]) {
            //verifica inimigo
            if(celulaCima.cells[localPeca-1].firstChild) {
                let inimigo = celulaCima.cells[localPeca-1].firstChild
                if(inimigo.getAttribute("cor") != minhaCor) {
                    inimigo.setAttribute("morto", "true")
                    let novoLocal = celulaCima.previousSibling.cells[localPeca-2]
                    if(!novoLocal.firstChild)  movimentos["cimaEsquerda"] = celulaCima.previousSibling.cells[localPeca-2]
                }
            } else {
...
function movePeca(peca, movimentos, local) {
    ...
    for (i in movimentos) {
        if (movimentos[i].isEqualNode(local)) {
            // testa inimigo
            if(movimentos.cimaEsquerda != undefined) {
                console.log(local)
                let inimigo = local.parentElement.nextSibling.cells[local.cellIndex+1]?.firstChild ?? null;
                console.log(inimigo)
                if (inimigo?.getAttribute("morto") == "true") deletaPeca(inimigo);
            }
